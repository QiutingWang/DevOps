...
- An introduction to Jenkins environment variables, parameters and use of groovy scripts. <https://blog.kipchirchirlangat.com/an-introduction-to-jenkins-environment-variables-parameters-and-use-of-groovy-scripts>
...
- Steps:
  - System->Global Tool configuration->find Maven, name it and download automatically
  - Available plugin: Maven Integration install
  - Finally, in new item creation page, we can find *Maven project* additional type.
...
- Required fields of Jenkinsfile
  - `pipeline`: put in the top
    ...
    - `parameters`:
      ...
      - it can be used in <u>when clause</u> by `params.name`
    ...
    - `stages`: where the work happens,start point
      - `stage("name")`
        ...
        - `steps`: include some command on the Jenkins server
          - `script`: we can define variables, functions... as groovy script.
...
- Access build tools for projects
  ...
  - for more information, plz refer to part6.
- Using External Groovy Script
  - `script` under steps block
  - import groovyFile as a global variable in init stage: `gv=load "Path"`. groovyFile的相对路径
  - Hence, if we need to use groovy script in other stages, we can simply use `variableName.functionName()`
    - use the variable/function name defined in the separate .groovy file.
    - in this case, the variableName is `gv` which we defined previously.
  - Basically, all the *environmental variables* and *expression* in Jenkinsfile can be included and available to .groovy script.
  - In **Replay** section, we can adjust and update the groovy script file, without changing in the original codes. 
    - Then both *original version and modified version* can be found in Replay part.
...
- Use Build Tools (yarn/gradle) in Jenkinsfile 
  - Method1: Global tools attribute
  - Method2: in stage using wrapper withGradle()
  - Details are written in Jenkinsfile 1.
